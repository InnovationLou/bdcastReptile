...
public class DanmuCatcher {
     ...
     * PC端直播间地址(改为批量输入)
     */
    public static final String url="https://live.douyin.com/193059181093";
...
//        String url=scanner.nextLine();
        try (Playwright playwright = Playwright.create()) {
            Browser browser = playwright.chromium().launch(
                    new BrowserType.LaunchOptions().setHeadless(false) //取消无头模式，才能看见浏览器操作
...
//                                System.out.println(messagesList);
            ...
            page.onResponse(new Consumer<Response>() {
                ...
                public void accept(Response response) {
                    if (response.url().matches("https://pull-flv-.*.douyincdn.com/third/.*.flv?.*")){
                        System.out.println("accept flv.....");
                        try {
